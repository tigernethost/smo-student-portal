...
function LoginForm() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      window.location.href = data.user?.onboarding_done === false ? '/onboarding' : '/dashboard'
    } catch (err) {
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
          ...
          <h1 style={{ fontSize: '1.4rem', fontWeight: '800', color: '#111827' }}>Welcome back</h1>
          <p style={{ fontSize: '0.85rem', color: '#6b7280', marginTop: '4px' }}>Sign in to your SchoolMATE account</p>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '2rem', boxShadow: '0 8px 32px rgba(0,0,0,0.08)', border: '1px solid #f3f4f6' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '1.25rem' }}>
            {SOCIAL_PROVIDERS.map(p => (
              <button key={p.id} onClick={() => handleSocial(p.id)} disabled={!!socialLoading || loading}
                ...
                {socialLoading === p.id ? <span style={{ fontSize: '1rem' }}>⏳</span> : p.icon}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1.25rem' }}>
            ...
            <span style={{ fontSize: '0.75rem', color: '#9ca3af', fontWeight: '500' }}>or sign in with email</span>
            <div style={{ flex: 1, height: '1px', background: '#e5e7eb' }} />
          </div>
          ...
          <p style={{ textAlign: 'center', fontSize: '0.82rem', color: '#6b7280', marginTop: '1.25rem' }}>
            ...
            <Link href="/register" style={{ color: '#2563eb', fontWeight: '700', textDecoration: 'none' }}>Sign up free</Link>
          </p>
        </div>
